...
const Movie = ({ movie, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center  items-left absolute left-5 bottom-16 lg:bottom-20 -z-1 flex-col">
        <div className="text-[15px] font-bold ">{movie.title}</div>
        <div className=" flex items-center  gap-1 text-[10px] ">
          <FaRegStar
            className="transition-all block duration-100 scale-130 text-yellow-300 bg-yellow-300 hover:bg-yellow-300 hover:scale-150 "
            style={{
              clipPath:
        ...
        <p className="text-[7px] pr-1 text-gray-400">
          {movie.overview?.slice(0, 70)}...
        </p>
      </div>
      <button
        className=" rounded-sm  h-6 absolute text-gray-400 font-bold -z-1 text-[12px] bottom-5 cursor-pointer "
